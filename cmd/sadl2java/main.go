...
func main() {
	...
	err = generatePojos(model, *pDir, *pSrc, *pPackage, *pLombok, *pGetters, *pJsonutil)
	if err != nil {
		fmt.Fprintf(os.Stderr, "*** %v\n", err)
		os.Exit(1)
	}
...
func generatePojos(model *sadl.Model, dir, src, pkg string, lombok, getters, jsonutil bool) error {
	gen := newPojoGenerator(model, dir, src, pkg, lombok, getters, jsonutil)
	if gen.jsonutil {
		gen.createJsonUtil()
	}
...
func newPojoGenerator(model *sadl.Model, dir, src, pkg string, lombok, getters, jsonutil bool) *PojoGenerator {
   gen := &PojoGenerator{
		model: model,
		dir: dir,
		src: src,
