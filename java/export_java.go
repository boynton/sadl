...
func defaultPackage(domainName string) string {
...
func NewGenerator(model *sadl.Model, outdir string, config *sadl.Data) *Generator {
	...
	gen.Name = sadl.Capitalize(name)
	pkg := gen.GetConfigString("package", "")
	if pkg == "" {
		pkg = defaultPackage(domain)
	}
	gen.Package = pkg
	...
	gen.Model = model
	gen.OutDir = outdir
	gen.EnsurePackageDir(gen.Package)
	if gen.GetConfigBool("model", true) {
		gen.ModelCode = true
		gen.ModelPackage = gen.GetConfigString("model-package", pkg+".model")
		gen.EnsurePackageDir(gen.ModelPackage)
	}
	...
	if gen.GetConfigBool("client", false) {
		gen.ClientCode = true
		gen.ClientPackage = gen.GetConfigString("client-package", pkg+".client")
		gen.EnsurePackageDir(gen.ClientPackage)
	}
	return gen
}
...
func (gen *Generator) EnsurePackageDir(pkg string) {
	if gen.Err == nil {
		srcpath := filepath.Join(gen.OutDir, gen.SourceDir)
...
func (gen *Generator) CreatePojo(ts *sadl.TypeSpec, className, comment string, exceptions map[string]string) {
	...
	if result != "" {
		...
		gen.WriteJavaFile(className, result, gen.ModelPackage)
	}
}
