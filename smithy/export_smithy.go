	...
	for _, hd := range schema.Http {
		...
		if len(hd.Inputs) > 0 {
			...
			for _, in := range hd.Inputs {
				...
				if in.Path {
					...
					mem.HttpHeader = in.Header
				} else {
					mem.HttpPayload = true
				}
				inShape.Members[in.Name] = mem
			}
			theNamespace.Shapes[shape.Input] = &inShape
		}
		...
		if len(hd.Expected.Outputs) > 0 {
			...
			for _, out := range hd.Expected.Outputs {
				...
				if out.Header != "" {
					mem.HttpHeader = out.Header
				} else {
					mem.HttpPayload = true
				}
				outShape.Members[out.Name] = mem
			}
			theNamespace.Shapes[shape.Output] = &outShape
		}
		...
		if len(hd.Exceptions) > 0 {
			for _, e := range hd.Exceptions {
				...
				if tmp, ok := theNamespace.Shapes[e.Type]; ok {
					tmp.HttpError = e.Status
				} else {
