...
func mapTypeReference(model *sadl.Model, ns string, shapes map[string]*Shape, prefix string, fd *sadl.StructFieldDef) string {
	ftype := capitalize(prefix) + capitalize(fd.Name)
	td := model.FindType(ftype)
	if td != nil {
		fmt.Printf("Inline defs not allowed, synthesize %q to refer to: %s\n", ftype, sadl.Pretty(fd))
...
func shapeFromArray(model *sadl.Model, ns string, shapes map[string]*Shape, tname string, ts *sadl.TypeSpec, annos map[string]string) Shape {
	member := Member{
...
func shapeFromMap(model *sadl.Model, ns string, shapes map[string]*Shape, tname string, ts *sadl.TypeSpec) Shape {
	key := Member{
